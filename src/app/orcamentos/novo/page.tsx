...
export default function NovoOrcamentoPage() {
  ...
  const availableVehicles = mockVehicles.filter((v) => v.customerId === form.customerId);

  const addItem = () => {
    setItems((prev) => [
      ...prev,
      { id: String(Date.now()), description: "", type: "peca", quantity: 1, unitPrice: 0 },
    ]);
  };
  ...
  const totalPecas = items.filter((i) => i.type === "peca").reduce((acc, i) => acc + i.quantity * i.unitPrice, 0);
  const totalMaoDeObra = items.filter((i) => i.type === "mao_de_obra").reduce((acc, i) => acc + i.quantity * i.unitPrice, 0);
  const total = totalPecas + totalMaoDeObra;

  const handleSave = async (redirect?: string) => {
    setSaving(true);
    await new Promise((r) => setTimeout(r, 800));
    if (redirect === "financiamento") {
      router.push("/financiamento?from=novo");
    } else {
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue placeholder={form.customerId ? "Selecione o veículo" : "Primeiro selecione o cliente"} />
                    </SelectTrigger>
                    <SelectContent>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Textarea
                    placeholder="Descreva o problema relatado pelo cliente e diagnóstico da oficina..."
                    rows={3}
              ...
              <CardContent className="space-y-3">
                ...
                <div className="pt-3 border-t space-y-2">
                  <div className="flex justify-between text-sm text-slate-600">
                    <span>Peças</span><span>{formatCurrency(totalPecas)}</span>
                  </div>
                  <div className="flex justify-between text-sm text-slate-600">
                    <span>Mão de obra</span><span>{formatCurrency(totalMaoDeObra)}</span>
                  </div>
                  <div className="flex justify-between font-bold text-slate-900 text-lg pt-2 border-t">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{formatCurrency(totalPecas)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">Mão de obra</span>
                    <span className="font-medium">{formatCurrency(totalMaoDeObra)}</span>
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <Button
                    ...
                    onClick={() => handleSave("financiamento")}
